...
import { GET_ALL_BRANDS_SEARCH_MAX_LENGTH } from "../../../../src/features/admin/brand/const/index.js";
	...
	describe("Get All Brands", () => {
		it("Should return 200 status code when request is successful", async () => {
			await withTestApp(async (app) => {
				...
				const res = await app.getAllBrands({
					headers: new Headers({
						Cookie: session,
					}),
				});
				...
				const testCases = [
					{
						...
						data: {
							search: faker.string.alpha({
								length: GET_ALL_BRANDS_SEARCH_MAX_LENGTH + 1,
							}),
						},
					},
					{
