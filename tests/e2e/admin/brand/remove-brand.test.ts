...
import {Brand, UserGender, UserRole} from "../../../../src/common/types/db.js";
...
describe("Admin", () => {
	...
	const setup = async (app: TestApp) => {
		...
		return {
			session,
			brands: (await getBrandsRes.body.json())
				.data as Selectable<Brand>[],
		};
	};

	describe("Remove brand", () => {
		it("Should return 200 status code when request is successful", async () => {
			await withTestApp(async (app) => {
				const {session, brands} = await setup(app);

				const res = await app.removeBrand(brands[0].id, {
					headers: new Headers({
						Cookie: session,
					}),
				});
		...
		it("Should be deleted from database when request is successful", async () => {
			await withTestApp(async (app) => {
				...
				expect(dbBrand).toBeUndefined();
			});
		});

		it("Should return 400 status code when brandId is invalid", async () => {
			await withTestApp(async (app) => {
				const session = await app.createAdminUser(signUpData);
				...
				await Promise.all(
					testCases.map(async ({name, id}) => {
						const res = await app.removeBrand(id as string, {
							headers: new Headers({
								Cookie: session,
							}),
						});
		...
		it("Should return 401 status code user is not authenticated", async () => {
			await withTestApp(async (app) => {
				const res = await app.removeBrand(faker.string.uuid(), {});
