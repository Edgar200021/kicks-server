...
describe("Admin", () => {
	...
	describe("Get Admin Product Sku", () => {
		it("Should return 200 status code when request is successful", async () => {
			await withTestApp(async (app) => {
				const {session, productSku} = await setup(app);

				const res = await app.getAdminProductSku(productSku.id, {
					headers: new Headers({
						Cookie: session,
					}),
				});
			...
			await withTestApp(async (app) => {
				...
				await Promise.all(
					testCases.map(async ({name, id}) => {
						const res = await app.getAdminProductSku(id as string, {
							headers: new Headers({
								Cookie: session,
							}),
						});
		...
		it("Should return 401 status code user is not authenticated", async () => {
			await withTestApp(async (app) => {
				const res = await app.getAdminProductSku(faker.string.uuid());
