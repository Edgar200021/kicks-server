...
import {Brand, Category, type DB, Product} from "@/common/types/db.js";
import {
	GetAllAdminProductsRequestQuery,
} from "@/features/admin/product/schemas/get-all-admin-products.schema.js";

export async function getAll(this: AdminProductRepository, query: GetAllAdminProductsRequestQuery): Promise<{
	count: number,
	...
	const products = await this.db.selectFrom("product as p")
		.leftJoin("category as c", "c.id", "p.categoryId")
		.leftJoin("brand as b", "b.id", "p.brandId")
		.select(["p.id", "p.createdAt", "p.updatedAt", "p.title", "p.description", "p.isDeleted", "p.gender", "p.tags", "c.id as categoryId", "c.name as categoryName", "b.id as brandId", "b.name as brandName"])
	...
	const {count} = await this.db.selectFrom("product as p")
		.leftJoin("category as c", "c.id", "p.categoryId")
		.leftJoin("brand as b", "b.id", "p.brandId")
